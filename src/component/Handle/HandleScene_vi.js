...
import scene from '../data/vi/Mazes.json'
import './handleLightcones.css'

function HandleScene_vi () {

  const [selectedOption, setSelectedOption] = useState(scene[0]);
  ...
  return (
    <>
      ...
      <div style={{display: 'flex'}}>
          <p className='name-of-selection'>Tên Cảnh: </p>
          <Select className='select'
            value={selectedOption}
            onChange={handleSelectChange}
            options={scene.map(scene=> ({ value: scene.id, label: scene.name }))}
